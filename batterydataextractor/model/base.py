# -*- coding: utf-8 -*-
"""
batterydataextractor.model.base
    ...
    def __set__(self, instance, value):
        ...
        if value is None:
            ...
            instance._values[self.name] = [
                self.field.process(v) for v in value]
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        for attr_name, attr_value in six.iteritems(attrs):
            if isinstance(attr_value, BaseType):
                # Set the name attribute on the Type to the attribute name on
                # the Model
...
class BaseModel(six.with_metaclass(ModelMeta)):
    ...
    def __eq__(self, other):
        # TODO: Check this actually works as expected (what about default
        # values?)
        ...
        for field_name, field in six.iteritems(self.fields):
            if hasattr(field, 'model_class'):
                if self[field_name] == field.default \
                   and field.required:
                    return False
                if field.required and hasattr(
                        self[field_name],
                        'required_fulfilled') and not self[field_name].required_fulfilled:
